...
describe('Oracle', () => {
  ...
  beforeEach('deploy contracts', async () => {
    usdt = await MockToken.connect(operator).deploy();
    cash = await Cash.connect(operator).deploy();
    share = await Share.connect(operator).deploy();

    await usdt.connect(operator).mint(operator.address, ETH.mul(2));
    await usdt.connect(operator).approve(router.address, ETH.mul(2));
    await cash.connect(operator).mint(operator.address, ETH);
    await cash.connect(operator).approve(router.address, ETH);

    await addLiquidity(provider, operator, router, cash, usdt, ETH);
