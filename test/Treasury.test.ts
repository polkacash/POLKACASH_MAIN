...
const INITIAL_SAC_AMOUNT = utils.parseEther('50000');
const INITIAL_SAS_AMOUNT = utils.parseEther('10000');
const INITIAL_SAB_AMOUNT = utils.parseEther('50000');
...
describe('Treasury', () => {
  ...
  describe('seigniorage', () => {
    describe('#allocateSeigniorage', () => {
      beforeEach('transfer permissions', async () => {
        await cash.mint(operator.address, INITIAL_SAC_AMOUNT);
        await cash.mint(treasury.address, INITIAL_SAC_AMOUNT);
        await share.mint(operator.address, INITIAL_SAS_AMOUNT);
  ...
  describe('bonds', async () => {
    beforeEach('transfer permissions', async () => {
      await cash.mint(operator.address, INITIAL_SAC_AMOUNT);
      await bond.mint(operator.address, INITIAL_SAB_AMOUNT);
      for await (const contract of [cash, bond, share, boardroom]) {
        await contract.connect(operator).transferOperator(treasury.address);
      }
    });
