...
import './interfaces/IPokaAsset.sol';
import './interfaces/IOracle.sol';
import './interfaces/IBoardroom.sol';
import './interfaces/IUniswapV2Router02.sol';

/**
 * @title Poka Cash Treasury contract
 * @notice Monetary policy logic to adjust supplies of Poka cash assets
...
contract Treasury is ContractGuard, Operator {
    ...
    modifier checkOperator {
        require(
            IPokaAsset(cash).operator() == address(this) &&
                IPokaAsset(bond).operator() == address(this) &&
                IPokaAsset(share).operator() == address(this) &&
    ...
    function initialize() public notInitialized checkOperator {
        ...
        IPokaAsset(cash).burn(IERC20(cash).balanceOf(address(this)));

        // mint only 1001 cash to itself
        IPokaAsset(cash).mint(address(this), 1001 ether);
        ...
        checkOperator
    {
        ...
        IPokaAsset(cash).burnFrom(msg.sender, amount);
        IPokaAsset(bond).mint(msg.sender, amount.mul(1e6).div(bondPrice));
        _updateCashPrice();
        ...
        checkOperator
    {
        ...
        IPokaAsset(bond).burnFrom(msg.sender, amount);
        ...
        checkOperator
    {
        ...
        if (seigniorageSaved.div(1e18) > bondDepletionFloor.div(1e6)) {
            IPokaAsset(cash).mint(address(this), seigniorage);
