...
const { pocPools, POOL_START_DATE } = require('./pools');
const fs = require('fs');
...
module.exports = async (deployer, network, accounts) => {
  ...
  const tokens = {};
  for await (const { contractName, token } of pocPools) {
    let decimals = ['USDC', 'USDT'].includes(token) ? 6 : 18;
    let contractAddress = knownContracts[token][network];
    if (!contractAddress) {
      const tokenContract = token === 'USDC' ? await MockToken.deployed() : await MockToken.new(token, token, decimals);
